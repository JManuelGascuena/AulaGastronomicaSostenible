...
// Control de sesión: Si no existe la variable 'usuario', lo mandamos al login
if (!isset($_SESSION['usuario'])) {
    header("Location: ./../login.php");
    exit();
}

// Si llega aquí, es que la sesión es correcta
?>
...
<head>
    ...
    <script src="./js/InicioAdministracion.js" defer></script>
</head>
...
try {
    ...
    // 3. Obtener los datos
    $sql = "SELECT huellaYoutube, titulo, publicado FROM Showcooking ORDER BY fechaCreacion DESC";
    $stmt = $pdo->query($sql);
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $videos[] = [
            ...
            'desc'      => $row['titulo'],
            'publicado' => $row['publicado'] // Guardamos el estado (0 o 1)
        ];
    }
            ...
            <div class="col">
                <div class="card h-100 shadow-sm custom-card">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <!-- Botón de previsualización (Modal) -->
                        <button type="button" 
                           ...
                           <i class="fa-solid fa-play"></i> Ver video en ventana modal
                        </button>

                        <!-- Nueva sección de administración -->
                        <div class="admin-controls border-top pt-3">
                            <!-- Checkbox Publicado -->
                            <div class="form-check mb-3">

                            <input class="form-check-input check-publicar" 
                               type="checkbox" 
                               id="pub-<?php echo $video['id']; ?>" 
                               data-id="<?php echo $video['id']; ?>"
                               <?php echo ($video['publicado'] == 1) ? 'checked' : ''; ?>
                               onchange="actualizarEstado(this)">
                               <label class="form-check-label text-dark" for="pub-<?php echo $video['id']; ?>">
                               Publicado
                            </label>
                            </div>
                            ...
                            <a href="editar_showcooking.php?id=<?php echo $video['id']; ?>" class="btn btn-warning btn-sm flex-fill">
                                <i class="fa-solid fa-pen-to-square"></i> Editar
                            </a>
                            <a href="./api/eliminar_showcooking.php?id=<?php echo $video['id']; ?>" class="btn btn-danger btn-sm flex-fill" onclick="return confirm('¿Estás seguro de eliminar este video?')">
...
<?php
        include './../pieDePagina-administracion.php'; // Carga el pie de página al final
?>   
